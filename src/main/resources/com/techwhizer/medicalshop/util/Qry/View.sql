...
SELECT tim.item_id,tim.is_stockable, tim.items_name, tim.unit, tim.strip_tab, tim.packing, tim.company_id, tim.mfr_id,
       tim.discount_id, tim.mr_id, tim.gst_id, tim.type, tim.narcotic, tim.item_type, tim.status, tim.created_by, tim.created_date,
       tim.is_active, tim.composition, tim.dose,tim. tag,(concat((tpt.igst + tpt.cgst + tpt.sgst), ' %')) as totalGst,
       coalesce(get_available_quantity(tim.item_id,'STRIP'),'0')as avl_qty_strip
        ,
       cast( coalesce(get_available_quantity(tim.item_id,'TAB'),'0') as numeric)
                                                                                                          as avl_qty_pcs,tpt.tax_id, tpt.sgst, tpt.cgst, tpt.igst,
       tpt.gstname, tpt.hsn_sac, tpt.description,td.department_id,td.department_code,department_name

from tbl_items_master as tim
         left join tbl_product_tax tpt on tpt.tax_id = tim.gst_id
         left join public.tbl_departments td on tim.department_code = td.department_code;
